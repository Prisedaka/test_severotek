...
const parse = () => {
  ...
  request.open('GET', '/view.xml', false);
...
const createTabs = (xmlData) => {
  ...
  for (let index = 0; index < views.length; index += 1) {
    ...
    const newTab = goog.dom.createDom(goog.dom.TagName.DIV, {
      class: classNames,
      id: tabName
    }, tabName);
...
const getDrawElement = {
  'ellipse': ({
    ...
    rx,
    ry
  }) => ellipseElement = graphics.drawEllipse(cx.value, cy.value, rx.value, ry.value, stroke, fill),
  'rectangle': ({
    x,
    y,
    width,
    height
  }) => rectElement = graphics.drawRect(x.value, y.value, width.value, height.value, stroke, fill),
  'triangle': ({
    points
  }) => {
    const [x1, y1, ...rest] = points.value.split(' ');
    const triangleData = graphics.createPath().moveTo(x1, y1).lineTo(rest);
  ...
  'brokenLine': ({
    ...
    const [x1, y1, ...rest] = points.value.split(' ');
    const brokenLineData = graphics.createPath().moveTo(x1, y1).lineTo(rest);
  ...
  'straightLine': ({
    ...
    const [x1, y1, ...rest] = points.value.split(' ');
    const straightLineData = graphics.createPath().moveTo(x1, y1).lineTo(rest);
  ...
  'text': ({
    ...
    x2,
    y2
  }) => textElement = graphics.drawTextOnLine(document.getElementById('text').value, x1.value, y1.value, x2.value, y2.value, 'right', font, stroke, fill),
}
